...
def moex_securities(market="shares", board=None, columns=None):
    engine, mkt, default_board = MOEX_MARKETS[market]
    board = board or default_board
    url = "%s/engines/%s/markets/%s/boards/%s/securities.json" % (MOEX_ISS, engine, mkt, board)
    if columns:
        url += "?securities.columns=%s" % ",".join(columns)
    d = iss_get(url)
    return iss_block_df(d["securities"])
...
# --- Data loading: OFZ and corporate bond universe ---
meta_full = pd.read_parquet("meta_full.parquet")
member = pd.read_csv("segment_membership.csv")

# Build seg_map from member
seg_map = member.set_index("secid")["segment"].to_dict()

# Load full history
full = pd.read_parquet("bond_history_full.parquet")
...
# Build segment panel
tr = pd.read_parquet("bond_tr_long.parquet")
...
# Fit the model
panel = pd.read_parquet("segment_returns_panel_tr.parquet")
...
for seed in range(30):
    ...
    try:
        ...
        ev, evec = np.linalg.eig(m.transmat_.T)
        stat = np.real(evec[:, np.argmin(np.abs(ev - 1))]); stat /= stat.sum()
...
transmat = reord(m.transmat_, o)
mu_raw = mu_raw[o]; Sigma_raw = Sigma_raw[o]; nu = nu[o]
port_vol_ann = port_vol_ann[o]
remap = {old: new for new, old in enumerate(o)}
hidden = np.array([remap[h] for h in hidden]); post = post[:, o]
ev, evec = np.linalg.eig(transmat.T)
stat = np.real(evec[:, np.argmin(np.abs(ev - 1))]); stat /= stat.sum()
selftr = np.diag(transmat); dur = 1 / (1 - selftr)
port_drift_bp = (mu_raw @ w) * 1e4
vit = np.array([(hidden == k).mean() for k in range(4)])

summ = pd.DataFrame({"regime": [f"R{i+1}" for i in range(4)], "nu": nu.round(2),
  "port_vol_ann_%": port_vol_ann.round(2), "port_mean_bp": port_drift_bp.round(1),
  "self_trans": selftr.round(3), "duration_d": dur.round(1),
  "stationary_%": (stat * 100).round(1), "viterbi_%": (vit * 100).round(1)})
print(summ.to_string(index=False))
print("\ntransmat:\n", np.round(transmat, 3))
print("LL:", round(m.loglik_, 2))
...
summ.to_csv("regime_summary_tr.csv", index=False)
